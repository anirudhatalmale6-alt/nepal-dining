...
import { canonicalImage } from './menuData';
...
function sanitise(raw: unknown): TeamMember[] | null {
  ...
  const list = raw
    ...
    .map((m, n): TeamMember => ({
      ...
      emoji: m.emoji || '👤',
      // Same reason as menuData: non-www 301s to www, and admin/team.php
      // saves photo URLs on the bare host.
      photo: canonicalImage(m.photo || ''),
